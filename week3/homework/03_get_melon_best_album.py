...
def get_melon_best_album(genre_array, play_array):
    ...
    for i in range(n):
        ...
        if genre not in genre_total_play_dict:
            ...
            genre_index_array_dict[genre] = [[i,play]]
        else:
            genre_total_play_dict[genre] += play
            genre_index_array_dict[genre].append([i,play])

    print(genre_index_array_dict)
    sorted_genre_play_array = sorted(genre_total_play_dict.items(), key=lambda item: item[1], reverse=True)
    print(sorted_genre_play_array)
    result = []
    for genre, value in sorted_genre_play_array:
        index_play_array = genre_index_array_dict[genre]
